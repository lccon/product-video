...
import com.lc.service.impl.TokenServiceImpl;
...
public class TokenUtil {
	...
	public static Token checkToken(String tokenString, String ussage) throws TokenExpired, TokenNotFound, TokenOverAuthed, TokenUssageNotMatched {
		...
		if (token == null) {
			throw new BusinessValidationException("找不到该 token： " + tokenString + " ，无法授权");
		}
		if (token.getExpiretime().compareTo(ZonedDateTime.now()) < 0) {
			throw new BusinessValidationException("该 token： " + tokenString + " 已失效，无法授权");
		}
		if (token.getCountAuth() >= token.getMaxCountAuth()) {
			throw new BusinessValidationException("该 token： " + tokenString + " 已超过最大授权次数，无法授权");
		}
		if (!ussage.equals(token.getUssage())) {
			throw new BusinessValidationException("该 token： " + tokenString + " 无法用于 ussage: " + ussage + "，无法授权");
		}
